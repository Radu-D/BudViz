---
theme: dashboard
title: Vizualizare Date de Sănătate - 2023
toc: false
---

# Vizualizare Date de Sănătate pentru Anul 2023

## Statistici Generale pentru 2023
...
const filteredActivityData = healthActivityData.filter(d => d.year === selectedYear);
const filteredSportData = healthSportData.filter(d => d.year === selectedYear);
...
const stepsChart = Plot.plot({
  ...
  marks: [
    Plot.barY(monthlyActivityData, { x: "month", y: "totalSteps", fill: "steelblue" }),
    Plot.line(monthlyActivityData, { x: "month", y: "totalSteps", stroke: "red", strokeWidth: 2 }),
    Plot.dot(monthlyActivityData, { x: "month", y: "totalSteps", fill: "red", r: 4 })
  ]
});
```
...
const stepsHistogram = Plot.plot({
  title: "Distribuția Pașilor per Lună (2023)",
  width: 1000,
  height: 600,
  y: { 
    grid: true, 
  ...
  marks: [
    Plot.rectY(monthlyActivityData, Plot.binX({ y: "count" }, { x: "totalSteps", fill: "green" })),
    Plot.ruleY([0])
  ]
});
```
...
const cumulativeChart = Plot.plot({
  title: "Cumulativ Pași per Lună (2023)",
  width: 1000,
  height: 600,
  x: { 
    label: "Luna", 
  ...
  marks: [
    Plot.line(cumulativeSteps, { x: "month", y: "cumulative", stroke: "orange", strokeWidth: 2 }),
    Plot.dot(cumulativeSteps, { x: "month", y: "cumulative", fill: "orange", r: 4 })
  ]
});
```
...
const heatmapChart = Plot.plot({
  title: "Intensitatea Pașilor per Lună (2023)",
  width: 1000,
  height: 600,
  x: { 
    label: "Luna", 
  ...
  marks: [
    Plot.cell(monthlyActivityData, { x: "month", y: "averageSteps", fill: "totalSteps" })
  ]
});
```
